...
func (w *Worker) SendDelete(msg string) error {
	msg_s := C.CString(string(msg))
	defer C.free(unsafe.Pointer(msg_s))

	r := C.worker_send_doc_delete(w.worker.cWorker, msg_s)
	if r != 0 {
		errStr := C.GoString(C.worker_last_exception(w.worker.cWorker))
		return errors.New(errStr)
	}
...
func (w *Worker) SendUpdate(msg string) error {
	msg_s := C.CString(string(msg))
	defer C.free(unsafe.Pointer(msg_s))

	r := C.worker_send_doc_update(w.worker.cWorker, msg_s)
	if r != 0 {
		errStr := C.GoString(C.worker_last_exception(w.worker.cWorker))
		return errors.New(errStr)
	}
	...
	/*svalue := C.worker_send_delete(w.worker.cWorker, msg_s)
	return C.GoString(svalue)*/
}
